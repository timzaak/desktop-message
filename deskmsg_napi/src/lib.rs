...
static TOKIO_RT: Lazy<Runtime> = Lazy::new(|| Runtime::new().expect("Failed to create Tokio runtime for NAPI module"));
...
pub fn discovery_mdns(service_name: String, seconds: u32) -> Result<Vec<NapiServiceInfo>> {
    let result = TOKIO_RT.block_on(async { deskmsg::discovery::discovery_mdns(&service_name, seconds as u64) });
        ...
        Ok(services) => {
            ...
            for service_info in services {
                ...
                let properties = service_info
                    .get_properties()
                    .iter()
                    .map(|p| NapiProperty { key: p.key().to_string(), value: p.val_str().to_string() })
                    .collect();
